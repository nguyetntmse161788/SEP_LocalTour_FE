...
export function OverviewAnalyticsView() {
  ...
  useEffect(() => {
    const fetchUserIds = async () => {
      try {
        const response = await fetch('https://api.localtour.space/api/User/getListByRole?roleName=Service%20owner', {
          headers: {
    ...
    const fetchUserRegistrations = async () => {
      try {
        const response = await fetch(
          'https://api.localtour.space/api/Statistic/GetTotalPlaceByMonthAsync?year=2024'
        );
  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      ...
      for (const userId of userIds) {
        try {
          const response = await fetch(`https://api.localtour.space/api/Statistic/GetPlaceByMonthAsync?year=2024&userId=${userId}`)
          const data = await response.json();
          ...
          if (data && data.list) {
            ...
            // Calculate the total approvals for the user
    ...
    <DashboardContent maxWidth="xl">
      ...
      <Grid xs={12} md={6} lg={12}>
        <AnalyticsWebsiteVisits
          title="Statistics on the number of places service owners created during the year"
